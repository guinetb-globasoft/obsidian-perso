...
➜ **Time-to-market accéléré** — dbt Core, DLT et les patterns génériques (macros, YAML-driven) réduisent drastiquement le temps de développement de chaque nouveau cas d'usage. Le delivery data passe de semaines à jours.
...
| 2     | Déploiement Snowflake          | **Couvert par la présente proposition (Périmètre 2)** |
...
Les intégrations avec les services Groupe lors du déploiement Snowflake (Private Link, SSO Cerbère/ILEX, Azure Key Vault, Usercube, Graylog) sont prises en charge par SoHoft dans le cadre du Périmètre 2, en coordination avec les équipes CAGIP et RSI CA Immobilier. Voir section 9 — Hypothèses structurantes pour le détail.
...
### 4.1.3 Intégrations transverses avec les services Groupe

Les intégrations avec les services Groupe lors du déploiement Snowflake (Périmètre 2) sont prises en charge par SoHoft, en coordination avec les équipes CAGIP et RSI CA Immobilier :

➜ Private Link AWS — coordination avec CAGIP

➜ SSO Cerbère / ILEX — coordination avec CAGIP

➜ BIOK, Tri Secret Secure Encryption, Azure Key Vault — coordination avec RSI CA Immobilier

➜ Intégration Usercube — coordination avec CAGIP / équipes habilitations

➜ Déversement Graylog — coordination avec CAGIP

Le DAT produit dans le Périmètre 1 formalise les spécifications d'intégration de ces services. Notre intervention dans le Périmètre 2 couvre l'effort d'intégration côté Snowflake. La configuration des services Groupe eux-mêmes (activation du Private Link côté AWS, provisionnement des comptes SSO, mise à disposition du Key Vault) reste à la charge des équipes Groupe concernées. Voir section 9 — Hypothèses structurantes.
...
➜ **Expert Snowflake senior** : contribution à la conception de l'architecture Snowflake dans le DAT (organisation logique, dimensionnement warehouses, modèle RBAC, sécurité Business Critical, FinOps natif Snowflake), puis déploiement complet de la plateforme et des intégrations Groupe à l'issue du Périmètre 2. Intervient en appui de l'Architecte Data aux jalons structurants, pas en continu — garantit l'alignement avec les best practices Snowflake sans alourdir le dispositif.
...
➜ **Expert Data Engineering** : implémentation des pipelines DLT, des modèles dbt, des tests, de l'orchestration Airflow.

Cette configuration garantit un interlocuteur unique pour votre équipe et limite les points de friction dans les échanges techniques, tout en s'appuyant sur une expertise Snowflake pointue aux moments où elle apporte le plus de valeur.
...
| Expert Snowflake senior | P1 + P2 | Appui à la conception du chapitre Snowflake du DAT (zones, warehouses, RBAC, sécurité BC, FinOps), déploiement complet de la plateforme et des intégrations Groupe |
| Expert Data Engineering | P1 + P3 | Cartographie, cadre FinOps, spécifications détaillées MVP, implémentation DLT, dbt, tests, orchestration Airflow |
...
**Note sur la période estivale :** le planning intègre par anticipation la coupure estivale du 20 juillet au 15 août (S12-S15). Aucune sollicitation des référents métier ni du sponsor n'est prévue durant cette période — pas de COPIL, pas d'atelier, pas de validation demandée. L'équipe SoHoft pourra mettre à profit cette période pour des activités de fabrication technique (itérations dbt, optimisations, tests, documentation) sans interaction utilisateur.

➜ **Verrouillage de la conception au 17 juillet (jalon J8).** Ce jalon reste un point structurant du planning : les règles de gestion Natio doivent être validées, les schémas cibles NPM Altaix acceptés par les référents métier, et les critères d'acceptation fixés avant la coupure. Ce verrouillage sécurise la qualité de la fabrication estivale et permet une reprise en S16 directement orientée vers la recette métier. Si des points de conception restent ouverts au 17 juillet, le risque d'itérations post-coupure augmente significativement du fait de la disponibilité réduite des interlocuteurs — un avenant de décalage pourra être proposé en COPIL 3.
...
➜ **Disponibilité du compte Snowflake.** Le contrat Snowflake est en place au niveau Groupe. L'activation du compte et la création des accès administrateurs doivent être effectives avant S7 pour ne pas retarder le démarrage du P2.

➜ **Coordination CAGIP et RSI.** Les intégrations Groupe (Private Link, SSO, Key Vault, Usercube, Graylog) sont portées par SoHoft mais nécessitent la réactivité des équipes CAGIP et RSI pour la configuration de leur côté. Une coordination précoce est nécessaire pour éviter les blocages.
...
➜ **Complexité des règles de gestion Java Natio.** Les règles de gestion Java identifiées dans Natio sont supposées d'une complexité standard, transposable en SQL dbt ou Python DLT (voir hypothèse 9.5).
...
➜ **Cadre FinOps = conception + déploiement.** Le cadre FinOps est conçu dans le Périmètre 1 (document de gouvernance, conventions de tagging, spécifications des resource monitors) et déployé techniquement dans Snowflake dans le cadre du Périmètre 2.
...
➜ **Accès aux bases sources pour le Périmètre 3.** Le développement des pipelines DLT nécessite un accès en lecture aux bases Oracle (NPM Altaix) et SQL Server (Natio) sur un environnement de développement ou de recette. À défaut d'un accès direct, une copie des données de référence avec un jeu cohérent et représentatif devra être mise à disposition avant le démarrage du Périmètre 3.
...
Les 15 jours chiffrés pour le Périmètre 2 reposent sur les hypothèses suivantes :
...
➜ **Services Groupe opérationnels et disponibles** pour les intégrations transverses : Private Link AWS disponible auprès de CAGIP, SSO Cerbère/ILEX CAGIP en service, Azure Key Vault administré par RSI CA Immobilier, Usercube en production, Graylog opérationnel. Notre chiffrage inclut l'effort d'intégration côté Snowflake mais repose sur la disponibilité et la réactivité (sous 3 jours ouvrés) des équipes CAGIP et RSI pour la configuration de leur côté.
...
|---|---|---|---:|---:|
| **Périmètre 1** | Cadrage & Conception | 1, 4, 5 | 34,5 j | **24 150 €** |
| **Périmètre 2** | Déploiement Snowflake | 2 | 15 j | **8 625 €** |
| **Périmètre 3** | Réalisation des Quick Wins | 3 | 19 j | **11 650 €** |
...
**Précision importante sur le Périmètre 2 (Déploiement Snowflake) :** notre intervention couvre le déploiement complet de la plateforme Snowflake : setup de base (compte, zones, warehouses, RBAC, environnements) et intégrations avec les services Groupe (Private Link, SSO Cerbère/ILEX, Azure Key Vault/BIOK, Usercube, Graylog). La coordination avec les équipes CAGIP et RSI CA Immobilier est incluse dans le chiffrage.
...
| | Kickoff et animation des ateliers cadrage | 8 | 700 | 5 600 |
| | Pilotage projet (coproj + COPIL) | 1,5 | 700 | 1 050 |
| **Lot 2 — DAT et architecture cible** | | | | |
| | Cartographie AS-IS macro | 5 | 700 | 3 500 |
| | Formalisation DAT (incluant intégration recommandations CISO) | 10 | 700 | 7 000 |
...
| | Présentations DG et CISO | 2 | 700 | 1 400 |
| **Total Périmètre 1** | | **34,5** | | **24 150 €** |

**Note sur le pilotage projet :** le pilotage est chiffré selon une règle mécanique — 1 coproj hebdomadaire de 30 min + 15 min de préparation (0,75 h × 8 semaines = 6 h), 2 COPIL de 1 h + 1 h de préparation chacun (2 h × 2 = 4 h), soit 10 h au total. Arrondi à 1,5 j. Les ateliers de conception, la rédaction des livrables et les présentations DG/CISO sont chiffrés séparément.
...
| Bloc fonctionnel | Phase | Charge (jours) | TJM (€) | Prix (€ HT) |
|---|---|---:|---:|---:|
| Setup | Compte Snowflake, databases, schemas, zones (raw/staging/core/marts) | 1 | 575 | 575 |
| Configuration | Warehouses par usage + paramétrage (auto-suspend, scaling policy) | 1 | 575 | 575 |
| Sécurité | Déploiement RBAC (functional + access roles) selon DAT | 1,5 | 575 | 862 |
| Environnements | Dev / Recette / Production + documentation | 1 | 575 | 575 |
| Private Link | Mise en place Private Link AWS avec CAGIP | 2 | 575 | 1 150 |
| SSO | Intégration SSO Cerbère / ILEX CAGIP | 2 | 575 | 1 150 |
| Chiffrement | Activation BIOK + Tri Secret Secure Encryption + Azure Key Vault (avec RSI) | 3 | 575 | 1 725 |
| Habilitations | Intégration Usercube pour la gestion des habilitations | 2 | 575 | 1 150 |
| Observabilité | Déversement des événements sécurité vers Graylog | 1 | 575 | 575 |
| Transverse | Pilotage & coordination | 0,5 | 575 | 288 |
| **Total Périmètre 2** | | **15** | | **8 625 €** |

**Périmètre de notre intervention :**

➜ Setup complet de la plateforme Snowflake sur l'environnement dédié à CA Immobilier.
...
➜ Intégration Private Link AWS en coordination avec CAGIP.

➜ Intégration SSO Cerbère / ILEX en coordination avec CAGIP.

➜ Activation BIOK + Tri Secret Secure Encryption + Azure Key Vault en coordination avec RSI CA Immobilier.

➜ Intégration Usercube pour la gestion des habilitations.

➜ Déversement des événements sécurité vers Graylog.
...
| | Modèles dbt core (2 faits + 2 dimensions, historisation mensuelle) | 2 | 550 | 1 100 |
| | **Sous-total Natio** | **10** | | **5 950** |
| **NPM Altaix** (SSIS / Oracle, complexité simple) | | | | |
| | Conception macro dbt générique + template YAML | 3 | 700 | 2 100 |
| | Instanciation des 7 cibles du Patrimoine via template | 2 | 550 | 1 100 |
| | **Sous-total NPM Altaix** | **5** | | **3 200** |
| **Transverse** | | | | |
| | Pilotage Périmètre 3 (coproj + COPIL) | 1 | 700 | 700 |
| | Intégration Airflow (orchestration des 2 cas d'usage) | 2 | 550 | 1 100 |
| | Coordination avec l'environnement Snowflake | 1 | 700 | 700 |
| | **Sous-total transverse** | **4** | | **2 500** |
| **TOTAL Périmètre 3** | | **19** | | **11 650 €** |

**Note sur le pilotage projet :** chiffrage aligné sur la même règle mécanique que le Périmètre 1 — 1 coproj hebdomadaire de 30 min + 15 min de préparation sur les 5 semaines pleines d'activité (S10, S11, S16, S17, S18), soit 3,75 h, + 2 COPIL de 1 h + 1 h de préparation chacun (COPIL 3 en S11 et COPIL 4 en S18), soit 4 h. Total 7,75 h, arrondi à 1 j. La coupure estivale S12-S15 est sans sollicitation client, donc sans pilotage projet.
...
|---|---:|---|
| Signature | 13 328 € | 30% du forfait global |
| DAT V1 livré — J5 (S6) | 8 885 € | 20% du forfait global |
| Validation CISO + DG, clôture P1 + P2 — J6/J7 (S8-S9) | 11 106 € | 25% du forfait global |
| Recette métier validée — J9 (S17) | 6 664 € | 15% du forfait global |
| Clôture globale — J10 (S18) | 4 442 € | 10% du forfait global |
| **Total** | **44 425 €** | |
